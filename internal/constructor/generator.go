...
func mergeGoSources2(sources ...[]byte) ([]byte, error) {
	if len(sources) == 0 {
		return nil, nil
	}
	...
	for i, src := range sources {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to parse src%d.go: %w", i, err)
		}
	...
	var header *ast.CommentGroup
	if files[0].Doc != nil && len(files[0].Doc.List) > 0 {
		newList := make([]*ast.Comment, len(files[0].Doc.List))
		for i, c := range files[0].Doc.List {
			newList[i] = &ast.Comment{
				Slash: token.Pos(1), // 清掉原位置
				Text:  c.Text,
			}
		}
		header = &ast.CommentGroup{List: newList}
	}
	...
	merged := &ast.File{
		...
		Decls: []ast.Decl{},
		Doc:   header,
		// Comments: append([]*ast.CommentGroup{}, files[0].Comments...),
		Scope: files[0].Scope,
		// Unresolved: nil,
	}
...
func mergeGoSources(sources ...[]byte) ([]byte, error) {
	...
	// 			if bytes.HasPrefix([]byte(c.Text), []byte("// Code generated by")) {
	// 				headerComments = append(headerComments, &ast.Comment{
	// 					Slash: token.NoPos,
	// 					Text:  c.Text,
	// 				})
	// 			}
	...
	// uniqHeader := []*ast.Comment{}
	// for _, c := range headerComments {
	// 	if !seenHeader[c.Text] {
	// 		seenHeader[c.Text] = true
	// 		uniqHeader = append(uniqHeader, c)
	// 	}
	// }
	// headerGroup := &ast.CommentGroup{List: files[0].Doc.List}
	...
	var buf bytes.Buffer
	// header
	if len(files[0].Doc.List) > 0 {
		for _, c := range files[0].Doc.List {
